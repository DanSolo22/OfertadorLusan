...
from ofertador.functions import set_repeat_table_header, insert_hr, comprovar_plazo, comprovar_stock, \
    crear_tabla_clientes, crear_tabla_resumen_pedido, insertar_barra_final_productos, string_to_float
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                if "_OFE_" in str(nombre_archivo):
                        ...
                        for row in csv_reader:
                            if count > 2:
                                ...
                                if str(row[23]).strip() == 'Texto':
                                    ...
                                    row_cells[3].text = string_to_float(row[18].strip())
                    ...
                    if idioma == 'INGLES':
                        condiciones.add_run('CONDITIONS:\n').font.size = Pt(11)
                        condiciones.add_run('\n').font.size = Pt(3)
                        condiciones.add_run('- Price and delivery time are as offered except for sale\n- This ').font.size = Pt(9)
                        ...
                        condiciones.add_run(', in case of a partial order, the prices may have to be revised.\n- The ').font.size = Pt(9)
                        ...
                        condiciones.add_run('- The special parts can be delivered with a +/-10% difference from the quoted quantity.').font.size = Pt(9)
                        condiciones.add_run('\n- The providing of the goods will depend on the acceptance of risk by ').font.size = Pt(9)
